...
func (ll LogLevel) String() (s string) {
    switch ll {
        case EmergencyLogLevel : s = "EMERGENCY"
        case AlertLogLevel     : s = "ALERT"
        case CriticalLogLevel  : s = "CRITICAL"
        case ErrorLogLevel     : s = "ERROR"
        case WarningLogLevel   : s = "WARNING"
        case NoticeLogLevel    : s = "NOTICE"
        case InfoLogLevel      : s = "INFO"
        case DebugLogLevel     : s = "DEBUG"
...
func (l *Log) String() string {
    ...
    for _, h := range l.Handlers {
        if h != nil { s += fmt.Sprint(h.String()) }
    }
    return s
}
...
func (l *Log) Debug(msg string) {
    for _, h := range l.Handlers { h.Debug(msg) }
}

func (l *Log) Info(msg string) {
    for _, h := range l.Handlers { h.Info(msg) }
}

func (l *Log) Notice(msg string) {
    for _, h := range l.Handlers { h.Notice(msg) }
}

func (l *Log) Warning(msg string) {
    for _, h := range l.Handlers { h.Warning(msg) }
}

func (l *Log) Error(msg string) {
    for _, h := range l.Handlers { h.Error(msg) }
}

func (l *Log) Critical(msg string) {
    for _, h := range l.Handlers { h.Critical(msg) }
}

func (l *Log) Alert(msg string) {
    for _, h := range l.Handlers { h.Alert(msg) }
}

func (l *Log) Emergency(msg string) {
    for _, h := range l.Handlers { h.Emergency(msg) }
}
...
func (l *Log) Close() {
    for _, h := range l.Handlers { h.Close() }
}

const logLength int = 5
func NewLog(num int) *Log {
    if num == 0 { num = logLength }
...
func NewFileHandler(filename string,
                    fmt string, lvl LogLevel) (*FileHandler, os.Error) {
    // open log file
 ...
 */
func (s *SyslogHandler) log(level LogLevel,  msg string) os.Error {
    if s.Level() >= level {
        s.Fac = FacLocal0
